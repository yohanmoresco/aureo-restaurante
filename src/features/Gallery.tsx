/**
 * File: src/features/Gallery.tsx
 * Purpose: Visual gallery showcasing the restaurant's environment, dishes, and atmosphere.
 *
...
/**
 * Renders the Gallery section/component.
      ...
      <div className="flex flex-col gap-20">
        ...
        <header className="text-center flex flex-col gap-6 items-center max-w-4xl mx-auto px-6">
          ...
          <FadeUp delay={0.1}>
            ...
            >
              A <span className="font-serif italic text-gold-2">Atmosfera</span>{" "}
              Áureo
            </h2>
          </FadeUp>
          <FadeUp delay={0.2}>
            <p className="text-text-2 text-base lg:text-lg font-light leading-relaxed max-w-2xl mx-auto">
              Cada detalhe da nossa casa foi pensado para criar memórias
              inesquecíveis. Um refúgio de sofisticação e conforto no coração da
              cidade.
            </p>
          </FadeUp>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 md:gap-4 px-1 md:px-4">
          {galleryImages.map((src, idx) => {
            ...
            if (idx === 0)
              spanClass = "aspect-square lg:aspect-[4/5] lg:row-span-2";
            if (idx === 1)
              spanClass = "aspect-square lg:aspect-video lg:col-span-2";
            ...
            return (
              <div
                key={idx}
                className={`relative overflow-hidden group w-full h-full ${spanClass}`}
              >
                <ImageReveal
                  src={src}
                  alt={`Galeria ${idx + 1}`}
