...
export default function Menu({ id }: { id: string }) {
  ...
  return (
    <Section id={id} className="bg-dark-2 border-t border-b border-border-light py-32" aria-labelledby="menu-heading">
      <div className="flex flex-col gap-20 max-w-6xl mx-auto relative min-h-[900px] lg:min-h-[850px]">
        ...
        <header className="text-center flex flex-col gap-6 items-center">
          <FadeUp>
            <span className="text-gold-2 text-[10px] md:text-sm font-semibold tracking-[0.3em] uppercase">Seleção do Chef</span>
          </FadeUp>
          <FadeUp delay={0.1}>
            <h2 id="menu-heading" className="text-4xl md:text-[3.5rem] font-headings font-light text-text-1">
              Nossa <span className="font-serif italic text-gold-2">Composição</span>
            </h2>
          </FadeUp>
          <FadeUp delay={0.2}>
        ...
        <FadeUp delay={0.3} className="flex flex-nowrap overflow-x-auto gap-12 md:justify-center border-b border-border-light pb-2 no-scrollbar px-6 md:px-0 relative" role="tablist" aria-label="Categorias do Menu">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {mockMenuItems[activeCategory as keyof typeof mockMenuItems].map((item, idx) => (
                <StaggerItem key={idx} className="flex flex-col gap-4 group p-5 sm:p-6 -mx-5 sm:-mx-6 rounded-xl hover:bg-dark-1/80 border border-transparent hover:border-gold-1/10 transition-all duration-500 hover:shadow-[0_10px_40px_-15px_rgba(201,168,78,0.12)]">
        ...
        <FadeUp delay={0.6} className="mt-8 flex justify-center z-10 sticky bottom-0 bg-gradient-to-t from-dark-2 to-transparent pt-12 pb-4">
