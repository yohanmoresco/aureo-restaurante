    ...
    <Section id={id} className="bg-dark-1 py-32" aria-labelledby="faq-heading">
      <div className="max-w-4xl mx-auto px-6 flex flex-col gap-16">
        ...
        <header className="text-center flex flex-col gap-6 items-center">
          <FadeUp>
            <span className="text-gold-2 text-[10px] md:text-sm font-semibold tracking-[0.3em] uppercase">Informações Úteis</span>
          </FadeUp>
          <FadeUp delay={0.1}>
            <h2 id="faq-heading" className="text-4xl md:text-[3.5rem] font-headings font-light text-text-1">
              <span className="font-serif italic text-gold-2">Perguntas</span> Frequentes
            </h2>
          </FadeUp>
        </header>
        ...
        <div className="flex flex-col border-t border-border-light">
          {faqs.map((faq, idx) => (
             <FadeUp delay={0.1 * idx} key={idx} className="border-b border-border-light overflow-hidden">
               <button
